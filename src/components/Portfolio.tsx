      ...
      <div className="container mx-auto px-6">
        ...
        <div ref={gridRef} className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {projects.map((project, index) => (
            <div 
              key={index} 
              className={`bg-card border border-border rounded-lg overflow-hidden hover:shadow-md transition-shadow duration-300 ${gridVisible ? 'scroll-visible' : 'scroll-hidden'}`}
            ...
            >
              ...
              <div className="p-4">
                ...
                <p className="text-muted-foreground mb-3 text-xs leading-relaxed line-clamp-3">{project.description}</p>
                
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {project.technologies.slice(0, 3).map((tech, techIndex) => (
                    <span key={techIndex} className="px-2 py-0.5 bg-secondary text-secondary-foreground text-[10px] rounded font-medium">
                      {tech}
                    </span>
                  ))}
                </div>
